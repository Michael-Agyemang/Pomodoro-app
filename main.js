...
function timer(seconds) {
  ...
  const progressChecker = setInterval(function () {
    ...
    if (seconds === 0) {
      ...
      start__pause.innerHTML = "Restart"
      clearInterval(progressChecker)
    }
  ...
  colors.forEach(colored => {
    if (colored.checked) {
      ...
      if (colored.id == "turquoise") {
        ...
        checkedBtns.style.backgroundColor = "#70F3F8"
        checkedBtns.style.removeProperty = " ";
      
      }
       else if (colored.id == "purple") {
        progress__ring.classList.add("purple__ring") 
         checkedBtns.style.backgroundColor = "#D881F8"
      } 
      else {
